...
import com.ks.androidtree.DPUtils;
...
public class SectionDecoration extends RecyclerView.ItemDecoration {
    ...
    public SectionDecoration(List<NameBean> dataList, Context context, DecorationCallback decorationCallback) {
        ...
        textPaint.setTextSize(DPUtils.dip2px(context, 14));
        textPaint.setColor(Color.DKGRAY);
        textPaint.setTextAlign(Paint.Align.LEFT);
        //设置分割线画笔
        dividePaint = new Paint();
        dividePaint.setColor(Color.DKGRAY);
        fontMetrics = new Paint.FontMetrics();
        //决定悬浮栏的高度等
        topGap = res.getDimensionPixelSize(R.dimen.sectioned_top);
        //决定文本的显示位置等
        paddingLeft = res.getDimensionPixelSize(R.dimen.sectioned_left);
        divideHeight = res.getDimensionPixelSize(R.dimen.sectioned_divide_height);
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (textLine == "") {
                ...
                if (position == 0 || isFirstInGroup(position)) {
                    ...
                    //绘制文本
//                    c.drawText(textLine, left + paddingLeft, bottom - (textPaint.getTextSize() + fontMetrics.descent) / 2, textPaint);
                } else {
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            c.drawText(textLine, left + paddingLeft, textY - lineHeight / 2, textPaint);
        }
    }
